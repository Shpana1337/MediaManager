...
class MM_body(QMainWindow):
    ...
    def opening_adding_tags(self, all_files_paths : list) -> None:
        ...
        with sqlite3.connect("database.db") as db:
            ...
            for _file_way in all_files_paths:
                _file_id = hex(stat(_file_way, follow_symlinks=False).st_ino)
                _file_tags = cursor.execute("SELECT tags FROM media_files WHERE id = ?", (_file_id,)).fetchone()
    ...
    def save_tags(self) -> None:
        ...
        for i in self.changed_tags_indexes_list:
            with sqlite3.connect("database.db") as db:
                ...
                _file_id = hex(stat(_file_path, follow_symlinks=False).st_ino)
                _file_tags = cursor.execute("SELECT tags FROM media_files WHERE id = ?", (_file_id,)).fetchone()
                ...
                if _file_tags:
                    ...
                    new_tags = new_tags[:-1:]
                    cursor.execute("UPDATE media_files SET tags = ? WHERE id = ?", (new_tags, _file_id))
                    db.commit()
    ...
    def add_tag(self, index : int) -> None:
        ...
        :param index: Индекс измененного медиафайла. Равен -1, когда пользователь хочет добавить тег нажатием 'Enter'
        """
        # Нажатие на кнопку +
        if index != -1:
            line_edit_text = self.line_edits_mass[index].text()
            self.changed_tags_indexes_list.add(index)
    ...
    def db_init(self):
        with sqlite3.connect('database.db') as db:
            cursor = db.cursor()
            cursor.execute(""" CREATE TABLE IF NOT EXISTS media_files (disk_name TEXT, id TEXT, tags TEXT) """)
            db.commit()
